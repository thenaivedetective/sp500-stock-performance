...
print("=" * 70)
print("  TRUE OUT-OF-SAMPLE VALIDATION — yFINANCE Q1 2025 TEST SET")
print("  Training : Q1 2010 – Q3 2024  (WRDS Compustat + CRSP)")
print("  Test     : Q4 2024 ratios → Q1 2025 outperformance (yFinance)")
...
print("\n[2/5] Pulling Q1 2025 returns from yFinance...")

q4_2024_rows = merged[merged['cal_quarter'] == pd.Period('2024Q4')].copy()
tickers_q4   = q4_2024_rows['tic'].dropna().unique().tolist()

TICKER_MAP = {
    'BRK.B': 'BRK-B', 'BRK.A': 'BRK-A',
    'BF.B':  'BF-B',  'BF.A':  'BF-A',
}
yf_tickers = [TICKER_MAP.get(t, t) for t in tickers_q4]
all_yf     = list(set(yf_tickers + ['SPY']))

print(f"  Downloading {len(all_yf)} tickers from yFinance (Dec 2024 – Mar 2025)...")
...
if isinstance(raw.columns, pd.MultiIndex):
    ...
    closes.columns = all_yf[:1]

closes.index = pd.to_datetime(closes.index)
closes_dec   = closes[closes.index.month == 12]
closes_mar   = closes[(closes.index.month == 3) & (closes.index.year == 2025)]

if len(closes_dec) == 0 or len(closes_mar) == 0:
    print("  ERROR: Could not retrieve Dec 2024 or Mar 2025 prices from yFinance.")
    exit(1)

p_dec = closes_dec.iloc[-1]
p_mar = closes_mar.iloc[-1]

spy_ret_q1 = float(p_mar['SPY'] / p_dec['SPY']) - 1
print(f"  SPY Q1 2025 return: {spy_ret_q1*100:+.2f}%")

yfin_map   = {t: yt for t, yt in zip(tickers_q4, yf_tickers)}
yf_returns = {}
for orig_tic, yf_tic in yfin_map.items():
    if yf_tic not in p_dec.index or yf_tic not in p_mar.index:
        continue
    ...
    yf_returns[orig_tic] = {
        'yf_return_q1_2025':      ret,
        'outperformer_yf_q1_2025': 1 if ret > spy_ret_q1 else 0,
    }

yf_df = pd.DataFrame.from_dict(yf_returns, orient='index').reset_index()
yf_df.columns = ['tic', 'yf_return_q1_2025', 'outperformer_yf_q1_2025']

pct_covered = len(yf_df) / len(tickers_q4) * 100
print(f"  Returns retrieved: {len(yf_df)}/{len(tickers_q4)} tickers ({pct_covered:.1f}%)")
print(f"  Q1 2025 outperformers: {yf_df['outperformer_yf_q1_2025'].sum()} / {len(yf_df)} "
      f"({yf_df['outperformer_yf_q1_2025'].mean()*100:.1f}%)")

print("\n[3/5] Building train and test sets...")

merged_test_raw = q4_2024_rows.merge(yf_df[['tic','outperformer_yf_q1_2025']], on='tic', how='inner')
merged_test_raw['outperformer_next'] = merged_test_raw['outperformer_yf_q1_2025']
...
merged_test = merged_test_raw.copy()

print(f"  Training rows : {len(merged_train):,}  (Q1 2010 – Q3 2024, WRDS labels)")
print(f"  Test rows     : {len(merged_test):,}   (Q4 2024 ratios → Q1 2025 yFinance labels)")
print(f"  Train outperformers: {merged_train['outperformer_next'].mean()*100:.1f}%")
print(f"  Test  outperformers: {merged_test['outperformer_next'].mean()*100:.1f}%")

print("\n[4/5] Fitting preprocessing on training data only...")
...
print("\n[5/5] Running validation across all 4 configurations...")
...
def run_validation(config_name, group_name, train_df, test_df, feature_cols=None):
    ...
    print(f"  Train N={len(train):,} (Q1 2010-Q3 2024) | Test N={len(test):,} (Q4 2024→Q1 2025 yFinance)")
    print(f"{'='*70}")
    print(tabulate([
    ...
    ], headers=["Metric","Train (WRDS 2010-24)","Test (yFinance Q1 2025)","Change"],
       tablefmt="github"))
    print(f"\n  Verdict: {verdict}")
...
print("\n\n" + "=" * 70)
print("  GRAND SUMMARY — TRUE OUT-OF-SAMPLE (Q1 2025 yFinance)")
print("  Q4 2024 financial ratios → Q1 2025 stock outperformance")
print("  All 4 configurations | All segments")
...
if len(df_val) > 0:
    ...
    print(tabulate([
        ...
        ["Test period",                                   "Q1 2025 (Jan–Mar 2025, yFinance)"],
        ["SPY Q1 2025 return",                            f"{spy_ret_q1*100:+.2f}%"],
        ["Test set size",                                 f"{len(merged_test):,} stocks (Q4 2024 ratios)"],
        ["Avg in-sample AUC (2010-24)",                   f"{overall_avg_train:.4f}"],
        ["Avg out-of-sample AUC (Q1 2025)",               f"{overall_avg_test:.4f}"],
    ...
    print(f"  Test horizon           : Q4 2024 ratios → Q1 2025 yFinance returns")
    print(f"  Method                 : VIF≤2.5 → PCA≥80% → Logistic Regression")
    print(f"  Winsorization          : 1st-99th percentile from training only")
    print(f"  Cluster assignment     : KMeans fitted on 2010-24 train, applied to Q4 2024")
    print(f"  Market cap thresholds  : Computed from training data only")
    print(f"  Forward-looking        : Q[t] ratios → Q[t+1] outperformance")
    print(f"  Benchmark source       : yFinance Adj Close prices, Jan–Mar 2025")
    print(f"  Benchmark index        : SPY (S&P 500 ETF) Q1 2025 total return")
    print("=" * 70 + "\n")

    df_val.to_csv('results_yfinance.txt', index=False, sep='\t')
    print("  Results saved to: results_yfinance.txt")
