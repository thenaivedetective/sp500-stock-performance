...
print("=" * 68)
print("  SECTOR-SEGMENTED MODEL — 2025 PREDICTIONS vs S&P 500")
print("  Best configuration from thesis: GICS Sector Segmentation")
print("=" * 68)
...
print(f"\nSectors found in training data: {len(sectors)}")
for s in sectors:
    n = (train_df['sector_name'] == s).sum()
    print(f"  {s}: {n} observations")

sector_models = {}
print("\n── TRAINING SECTOR MODELS ────────────────────────────────────")
for sector in sectors:
    s_train = train_df[train_df['sector_name'] == sector].copy()
    available = [c for c in ratio_cols if c in s_train.columns]
    s_clean   = s_train[available + ['outperformer_next']].dropna()
    if len(s_clean) < 50 or s_clean['outperformer_next'].nunique() < 2:
        print(f"  {sector}: skipped (insufficient data)")
        continue
    X_tr = s_clean[available]
    y_tr = s_clean['outperformer_next']
    try:
...
for col in ratio_cols:
    if col in comp_2025f.columns and col in winsor_bounds:
        lo, hi = winsor_bounds[col]
        comp_2025f[col] = comp_2025f[col].clip(lo, hi)
...
print("\n── GENERATING 2025 PREDICTIONS BY SECTOR ────────────────────")
for feat_qtr, label_qtr in test_quarters:
    comp_test = comp_2025f[comp_2025f['quarter'] == feat_qtr].copy()
    crsp_sub  = crsp_2025_sub[crsp_2025_sub['cal_qtr'] == label_qtr].copy()
    if crsp_sub.empty or comp_test.empty:
        continue

    spy_return = crsp_sub['spy_return'].iloc[0]
    q_label    = quarter_labels[label_qtr]
    print(f"\n  Quarter: {q_label}  |  SPY return: {spy_return*100:+.2f}%")

    quarter_preds = []
    for sector, model in sector_models.items():
        s_test = comp_test[comp_test['sector_name'] == sector].copy()
        if s_test.empty:
            continue
        s_merged = s_test.merge(crsp_sub[['gvkey','outperformer_next','actual_return','spy_return']],
                                on='gvkey', how='inner')
        if s_merged.empty:
            continue
        kept = model['kept']
        avail_kept = [c for c in kept if c in s_merged.columns]
        if len(avail_kept) < len(kept):
            continue
        for col in avail_kept:
            s_merged[col] = s_merged[col].fillna(train_medians.get(col, 0))
        X_te_sc  = model['scaler'].transform(s_merged[kept])
        X_te_pca = model['pca'].transform(X_te_sc)
        probs    = model['clf'].predict_proba(X_te_pca)[:, 1]
        preds    = (probs >= model['threshold']).astype(int)

        s_merged['prob_outperform'] = probs.round(4)
        s_merged['predicted']       = preds
        s_merged['predicted_label'] = preds
        quarter_preds.append(s_merged)

    if not quarter_preds:
        continue

    q_df = pd.concat(quarter_preds, ignore_index=True)
    q_df['feature_quarter'] = f"2025-Q{feat_qtr}"
    q_df['label_quarter']   = q_label
    q_df['predicted_label'] = q_df['predicted'].map({1: 'Outperformer', 0: 'Underperformer'})
    q_df['actual_label']    = q_df['outperformer_next'].map({1: 'Outperformer', 0: 'Underperformer'})
    q_df['correct']         = (q_df['predicted'] == q_df['outperformer_next']).astype(int)
    all_preds.append(q_df)

    pred_out = q_df[q_df['predicted'] == 1]['actual_return']
    pred_und = q_df[q_df['predicted'] == 0]['actual_return']
    acc      = q_df['correct'].mean() * 100

    port_return = pred_out.mean() if len(pred_out) > 0 else np.nan
    beat_spy    = port_return > spy_return if not np.isnan(port_return) else False

    print(f"    Stocks predicted: {len(q_df)} | Outperformers: {len(pred_out)} | Underperformers: {len(pred_und)}")
    print(f"    Accuracy: {acc:.1f}%")
    print(f"    Model portfolio return (equal-weight outperformers): {port_return*100:+.2f}%")
    print(f"    SPY return:                                          {spy_return*100:+.2f}%")
    print(f"    Model {'BEATS' if beat_spy else 'TRAILS'} SPY by {abs(port_return - spy_return)*100:.2f}%")
...
if all_preds:
    ...
    out_cols  = ['label_quarter','feature_quarter','tic','conm','sector_name',
                 ...
                 'prob_outperform','actual_return','spy_return']
    out_cols  = [c for c in out_cols if c in final_df.columns]
    final_df  = final_df[out_cols].sort_values(['label_quarter','prob_outperform'], ascending=[True,False])
    final_df  = final_df.rename(columns={
        'tic': 'Ticker', 'conm': 'Company', 'sector_name': 'Sector',
        'predicted_label': 'Predicted', 'actual_label': 'Actual',
        'correct': 'Correct', 'prob_outperform': 'P(Outperform)',
        'actual_return': 'Actual_Return', 'spy_return': 'SPY_Return',
        'label_quarter': 'Label_Quarter', 'feature_quarter': 'Feature_Quarter',
    })
    final_df.to_csv('sector_predictions_2025.csv', index=False)
    print(f"\n  Saved {len(final_df)} predictions → sector_predictions_2025.csv")

if portfolio_perf:
    perf_df = pd.DataFrame(portfolio_perf)
    perf_df.to_csv('portfolio_vs_spy_2025.csv', index=False)
    print("\n" + "=" * 68)
    print("  PORTFOLIO PERFORMANCE SUMMARY — SECTOR MODEL vs S&P 500")
    print("=" * 68)
    total_alpha = 0
    for _, row in perf_df.iterrows():
        symbol = "▲ BEATS" if row['Beat_SPY'] else "▼ TRAILS"
        print(f"\n  {row['Quarter']}")
        print(f"    Predicted outperformers (portfolio): {row['N_Predicted_Outperformers']} stocks")
        print(f"    Model portfolio return : {row['Model_Portfolio_Return_Pct']:+.2f}%")
        print(f"    SPY return             : {row['SPY_Return_Pct']:+.2f}%")
        print(f"    Alpha vs SPY           : {row['Alpha_vs_SPY_Pct']:+.2f}%  {symbol}")
        print(f"    Classification accuracy: {row['Accuracy_Pct']:.1f}%")
        total_alpha += row['Alpha_vs_SPY_Pct']
    avg_alpha = total_alpha / len(perf_df)
    beats = perf_df['Beat_SPY'].sum()
    print(f"\n  OVERALL ({beats}/{len(perf_df)} quarters beat SPY)")
    print(f"  Average alpha vs SPY: {avg_alpha:+.2f}% per quarter")
    print(f"  Saved portfolio summary → portfolio_vs_spy_2025.csv")
    print("=" * 68)
